...
#define DUTY_CYCLE_PERCENT 50 // Duty cycle deseado en porcentaje
...
#define DUTY_RESOLUTION    LEDC_TIMER_11_BIT  // Mayor resolución para mejor control del duty cycle
...
bool desaceleracion_activa = false;
unsigned long tiempo_anterior = 0;
unsigned long tiempo_serial = 0;

// Variables para control de desaceleración
float omega_D_des_i = 0.0;
float omega_D_des_f = 0.0;
float a_D_des = 0.0;
float omega_B_des_i = 0.0;
float omega_B_des_f = 0.0;
float a_B_des = 0.0;
float T_des = 5.0; // Tiempo de desaceleración en segundos

// Variables para dirección de motores
int dir_disco = HIGH; // Dirección por defecto
int dir_bola = HIGH;
...
void actualizarFrecuencias() {
    ...
    // Actualizar las velocidades angulares globales
 ...
 // Calcular duty cycle fijo al 50%
    uint32_t duty_max = (1 << DUTY_RESOLUTION) - 1;
    uint32_t duty_D = duty_max / 2; // 50% del duty_max
    uint32_t duty_B = duty_max / 2; // 50% del duty_max
    ...
    // Mostrar información cada medio segundo
...
void actualizarFrecuenciasDesaceleracion() {
    // Incrementar el tiempo
    t_global += dt;

    // Calcular las velocidades angulares actuales
    float omega_D_temp = omega_D_des_i + a_D_des * t_global;
    float omega_B_temp = omega_B_des_i + a_B_des * t_global;

    // Limitar a las velocidades finales (no menos de cero)
    if (omega_D_temp < omega_D_des_f) omega_D_temp = omega_D_des_f;
    if (omega_B_temp < omega_B_des_f) omega_B_temp = omega_B_des_f;
...
void mantenerVelocidadConstante() {
    // En esta fase, omega_D y omega_B son constantes
    // Las frecuencias se mantienen constantes

    float omega_D_temp = omega_D;
    float omega_B_temp = omega_B;
    ...
    // Calcular velocidades lineales
    float v_D = omega_D_temp * (R / 100.0);
    float v_B = omega_B_temp * (r / 100.0);
...
void setup() {
    ...
    digitalWrite(GPIO_DISCO_DIR, dir_disco); // Usar la variable dir_disco
    digitalWrite(GPIO_BOLA_DIR, dir_bola);   // Usar la variable dir_bola
    ...
    if (aceleracion_activa) {
        ...
        if ((tiempo_actual - tiempo_anterior) >= (dt * 1000)) {
            ...
            if (t_global >= T) {
                ...
                omega_D = omega_D_f; // Actualizamos la velocidad actual a la final alcanzada
                omega_B = omega_B_f;
                a_D = 0.0; // No más aceleración
                a_B = 0.0;
                t_global = 0.0; // Reiniciar el tiempo para la fase de velocidad constante
                Serial.println("Aceleración completada. Manteniendo velocidad constante.");
            }
        }
    } else if (desaceleracion_activa) {
        // Control de desaceleración
        if ((tiempo_actual - tiempo_anterior) >= (dt * 1000)) {
            tiempo_anterior = tiempo_actual;
            actualizarFrecuenciasDesaceleracion();
            // Verificar si se alcanzó el tiempo total de desaceleración
            if (t_global >= T_des) {
                desaceleracion_activa = false;
                Serial.println("Desaceleración completada.");
                // Detener motores
...
void procesarComando(char *comando) {
    if (strncmp(comando, "SET", 3) == 0) {
        ...
        while (token != NULL) {
            ...
            } else if (strcmp(token, "DIR_DISCO") == 0) {
                token = strtok(NULL, " =");
                if (token != NULL) {
                    ...
                    digitalWrite(GPIO_DISCO_DIR, dir_disco);
                    Serial.print("DIR_DISCO actualizado a: "); Serial.println(dir_disco == HIGH ? "HIGH" : "LOW");
                }
            } else if (strcmp(token, "DIR_BOLA") == 0) {
                token = strtok(NULL, " =");
                if (token != NULL) {
                    int valor = atoi(token);
                    if (valor == 0) {
                    ...
                    digitalWrite(GPIO_BOLA_DIR, dir_bola);
                    Serial.print("DIR_BOLA actualizado a: "); Serial.println(dir_bola == HIGH ? "HIGH" : "LOW");
                }
            }
            token = strtok(NULL, " =");
        }
        ...
        if (!aceleracion_activa) {
            ...
            Serial.println("Iniciando aceleración...");
        } else {
            Serial.println("La aceleración ya está en curso.");
        }
    } else if (strncmp(comando, "STOP", 4) == 0) {
        if (!desaceleracion_activa && (aceleracion_activa || omega_D > 0 || omega_B > 0)) {
            // Iniciar desaceleración suave
            omega_D_des_i = omega_D; // Usar la velocidad angular actual del disco
            omega_D_des_f = 0.0; // Queremos llegar a cero
            a_D_des = (omega_D_des_f - omega_D_des_i) / T_des; // Aceleración negativa

            omega_B_des_i = omega_B; // Usar la velocidad angular actual de la bola
            omega_B_des_f = 0.0;
            a_B_des = (omega_B_des_f - omega_B_des_i) / T_des;

            t_global = 0.0; // Reiniciamos el tiempo para la desaceleración
            tiempo_anterior = millis();
            tiempo_serial = millis();
            desaceleracion_activa = true;
            aceleracion_activa = false; // Detenemos la aceleración si estaba activa

            Serial.println("Iniciando desaceleración suave...");
        } else {
            Serial.println("Motores ya están detenidos o desacelerando.");
        }
        ...
        Serial.print("DIR_DISCO: "); Serial.println(dir_disco == HIGH ? "HIGH" : "LOW");
        Serial.print("DIR_BOLA: "); Serial.println(dir_bola == HIGH ? "HIGH" : "LOW");
        Serial.println("=====================");
    } else {
